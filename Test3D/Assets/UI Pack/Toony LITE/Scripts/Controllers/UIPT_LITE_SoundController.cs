...
// ######################################################################
// UIPT_LITE_SoundController class
//
// Simple Play/stop BG music and sounds.
// ######################################################################

public class UIPT_LITE_SoundController : MonoBehaviour
{
	// ########################################
	// Variables
	// ########################################
	...
	private static UIPT_LITE_SoundController instance;

	// Public static reference that can be accesd from anywhere
	public static UIPT_LITE_SoundController Instance
	{
		get
		{
			...
			if (instance == null)
			{
				instance = GameObject.FindObjectOfType<UIPT_LITE_SoundController>();
	...
	void InitAudioListener()
	{
		...
		{
			if (child.gameObject.GetComponent<UIPT_LITE_SoundController>() == null)
			{
				Destroy(child);
			}
		}
