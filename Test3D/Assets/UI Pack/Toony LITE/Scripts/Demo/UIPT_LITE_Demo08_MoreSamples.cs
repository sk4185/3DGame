...
// ######################################################################
// UIPT_LITE_Demo08_MoreSamples class
//
// Handles "Demo 08 - Landscape - More Samples" and "Demo 08 - Portrait - More Samples" scenes.
// ######################################################################

public class UIPT_LITE_Demo08_MoreSamples : MonoBehaviour
{
	// ########################################
	// Variables
	// ########################################
	...
	// UIs
	public GUIAnimFREE[] m_Samples = null;
	...
	void Start()
	{
		...
		GUIAnimSystemFREE.Instance.MoveIn(m_PanelBottom.transform, true);
	...
	void Update()
	{
		...
		if (m_ButtonWaitTimeCount > 0)
		{
			...
			if (m_ButtonWaitTimeCount <= 0)
			{
				GUIAnimSystemFREE.Instance.EnableButton(m_ArrowLeftButton.transform, true);
				m_ArrowLeftButton.interactable = true;

				GUIAnimSystemFREE.Instance.EnableButton(m_ArrowRightButton.transform, true);
	...
	public void Button_Home()
	{
		...
		GUIAnimSystemFREE.Instance.DontDestroyParticleWhenLoadNewScene(m_Samples[m_SampleUI_Index].transform, true);

		// Play Move Out animation
		GUIAnimSystemFREE.Instance.MoveOut(m_Samples[m_SampleUI_Index].transform, true);

		// Play Move Out animation
		GUIAnimSystemFREE.Instance.MoveOut(m_PanelBottom.transform, true);
	...
	public void Button_LeftArrow()
	{
		...
		GUIAnimSystemFREE.Instance.EnableButton(m_ArrowLeftButton.transform, false);
		m_ArrowLeftButton.interactable = false;
		GUIAnimSystemFREE.Instance.EnableButton(m_ArrowRightButton.transform, false);
	...
	IEnumerator ShowNextSample()
	{
		if (m_SampleUI_IndexOld >= 0 && m_SampleUI_IndexOld <= m_Samples.Length)
		{
			...
			// Stop particles in the hierarchy of given transfrom
			GUIAnimSystemFREE.Instance.StopParticle(m_Samples[m_SampleUI_IndexOld].transform);
		}
		...
		if (m_SampleUI_Index >= 0 && m_SampleUI_Index <= m_Samples.Length)
		{
			...
			GUIAnimSystemFREE.Instance.PlayParticle(m_Samples[m_SampleUI_Index].transform);
		}
